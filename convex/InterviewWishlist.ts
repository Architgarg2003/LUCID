...
export const AddToWishlist = mutation({
    ...
    handler: async (ctx, args) => {
        try {
            const newTestAnswerId = await ctx.db.insert("InterviewWishlist", {
                userId: args.userId,
                cardId: args.cardId
            });
...
export const RemoveFromWishlist = mutation({
    ...
    handler: async (ctx, args) => {
        try {
            ...
            const matchingEntries = await ctx.db.query("InterviewWishlist")
                .filter((q) => q.eq(q.field("userId"), args.userId))
                .filter((q) => q.eq(q.field("cardId"), args.cardId))
                .collect();
...
export const inBucket = query({
    ...
    handler: async (ctx, args) => {
        try {
            // Query for all matching documents
            const inWishlist = await ctx.db.query("InterviewWishlist")
                .filter((q) => q.eq(q.field("userId"), args.userId))
                .filter((q) => q.eq(q.field("cardId"), args.cardId))
                .first();
...
export const getUserBucket = query({
    ...
    handler: async (ctx, args) => {
        try {
            ...
            const entry = await ctx.db.query("InterviewWishlist")
                .filter((q) => q.eq(q.field("userId"), userId))
                .collect();

                return entry;
        } catch (error) {
            console.error("Error in userbucket query:", error);
            throw new Error("Failed to retrieve userBucket");
        }
    }
});
