...
export const getAllInterviewGeneratedCards = query({
    handler: async (ctx) => {
        try {
            const cards = await ctx.db.query("generatedInterviewCards").collect();
            ...
            console.error("Error in getAllInterviewGeneratedCards query:", error);
            throw new Error(`Failed to retrieve Interview generated cards: ${error.message}`);
        }
    },
});