...
export const similarCards = action({
    ...
    handler: async (ctx, args) => {
        ...
        const results = await ctx.vectorSearch("generatedInterviewCards", "by_embedding", {
        ...
        const fullResults:any = await Promise.all(
            results.map(async (result) => {
                const card = await ctx.runQuery(internal.GeneratedInterviewCardSearch.getCardById,({ cardId: result._id }));
